...
#define ARROW_RETURN_IF(condition, status) ARROW_RETURN_IF_(condition, status, ARROW_STRINGIFY(status))
...
namespace arrow
{
    ...
    class ARROW_EXPORT [[nodiscard]] Status : public util::EqualityComparable<Status>,
      ...
      public:
        ...
        template <typename... Args>
        static Status FromDetailAndArgs(StatusCode code, std::shared_ptr<StatusDetail> detail, Args&&... args) {
        ...
        static Status ExpressionValidationError(Args&&... args) {
            return Status::FromArgs(StatusCode::ExpressionValidationError, std::forward<Args>(args)...);
        }
        ...
        constexpr bool IsSerializationError() const { return code() == StatusCode::SerializationError; }
        ...
        constexpr bool IsExpressionValidationError() const { return code() == StatusCode::ExpressionValidationError; }
