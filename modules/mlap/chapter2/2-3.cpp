...
int main() {
    ...
    cv::warpAffine(temp_image, translation_image, warp_matrix_translation, temp_image.size(), INTER_LINEAR,
                   BORDER_TRANSPARENT);
    ...
    cv::copyMakeBorder(temp_image, temp_image, border, border, border, border, BORDER_CONSTANT,
                       cv::Scalar(200));  // 扩边
    ...
    border = 40;
    cv::copyMakeBorder(temp_image, temp_image_vertical, 10, 2 * border, border, 10 + border, BORDER_CONSTANT,
                       cv::Scalar(200));
    cv::copyMakeBorder(temp_image, temp_image_horizontal, border + 10, border, 10, 2 * border, BORDER_CONSTANT,
                       cv::Scalar(200));
    ...
    // 根据旋转矩阵进行仿射变换
    cv::warpAffine(temp_image_vertical, shear_vertical_image, warp_matrix_shear_vertical, temp_image_vertical.size(),
                   INTER_LINEAR, BORDER_REPLICATE);
    cv::warpAffine(temp_image_horizontal, shear_horizontal_image, warp_matrix_shear_horizontal,
                   temp_image_horizontal.size(), INTER_LINEAR, BORDER_REPLICATE);
