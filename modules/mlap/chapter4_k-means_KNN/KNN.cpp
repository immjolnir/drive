...
int main(int argc, char* argv[]) {
    // 1.读取原始数据
    string minist_digits_png(argv[1]);  // 使用图片格式的MNIST数据集（部分）
    Mat img = imread(minist_digits_png, 1);
    ...
    knn->setIsClassifier(true);  // 用于分类. CLASSIFIER
    // knn->setIsClassifier(false);  // 用于回归,REGRESSION
...
void generateDataSet(Mat& img, Mat& trainData, Mat& testData, Mat& trainLabel, Mat& testLabel, int train_rows) {
    ...
    for (int i = 1; i <= col_sample; i++)  // 50行：1-50行数字图像
    {
        ...
        for (int j = 1; j <= row_sample; j++)  // 100列：1-100列数字图像
        {
            ...
            temp_single_num =
              img(roi).clone();  // 注意此处需要使用深拷贝.clone()，后面才能改变切片图的形状，否则roi内存区域不连续
    ...
    printf("标签数据已生成\n\n");
    // printf("trainLabel(1,400,401,800,801,4000) = %f, %f, %f, %f, %f, %f\n", trainLabel.at<float>(0),
    // trainLabel.at<float>(399), trainLabel.at<float>(400), trainLabel.at<float>(799), trainLabel.at<float>(800),
    // trainLabel.at<float>(3999)); printf("testLabel (1,100,101,200,201,1000) = %f, %f, %f, %f, %f, %f\n",
    // testLabel.at<float>(0), testLabel.at<float>(99), testLabel.at<float>(100), testLabel.at<float>(199),
    // testLabel.at<float>(200), testLabel.at<float>(999)); cv::waitKey();
}